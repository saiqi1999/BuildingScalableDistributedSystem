...
public class QueryProcess implements Runnable {
    ...
    public void run() {
        ...
        for (int i = 0; i < scheduledTimes; i++) {
            ...
            int resortId = Math.abs(random.nextInt())%10;//uri pos 1
            int seasonsId = Math.abs(random.nextInt())%10;//uri pos 2
            int day = 3;//Math.abs(random.nextInt()) % 365;//uri pos 3 todo CHANGED IN HW4

            int waitTime = Math.abs(random.nextInt()) % 11;//request param 3
            int liftId = Math.abs(random.nextInt()%100);//request para 2, related to vertical according to algo.
